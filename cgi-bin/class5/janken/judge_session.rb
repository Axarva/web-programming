...
if cgi.params['cookiereset'].first
  ...
  print <<EOF_RESET
    <html><body>
    <h1>勝敗をリセットしました！</h1>
    <p><a href="janken_session.rb">もう一度勝負する？</a></p>
    </body></html>
EOF_RESET
  exit
end
...
# Game logic
win_count = (session["wins"] || 0).to_i
loss_count = (session["losses"] || 0).to_i
draw_count = (session["draws"] || 0).to_i
...
session['wins'] = win_count
session['losses'] = loss_count
session['draws'] = draw_count
